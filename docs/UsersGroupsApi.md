...
> update_group_template(id => $id, group_template_resource => $group_template_resource)
...
eval { 
    $api_instance->update_group_template(id => $id, group_template_resource => $group_template_resource);
};
if ($@) {
...
eval { 
    my $result = $api_instance->update_groups(size => $size, page => $page, order => $order);
    print Dumper($result);
};
if ($@) {
