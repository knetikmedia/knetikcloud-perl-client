...
eval { 
    my $result = $api_instance->get_users_using_get(filter_role => $filter_role, filter_displayname => $filter_displayname, filter_email => $filter_email, filter_firstname => $filter_firstname, filter_fullname => $filter_fullname, filter_lastname => $filter_lastname, filter_username => $filter_username, size => $size, page => $page, order => $order);
    print Dumper($result);
};
if ($@) {
