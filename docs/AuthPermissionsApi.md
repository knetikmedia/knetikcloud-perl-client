...
```perl
use KnetikCloud::Object::AuthPermissionsApi;
```
...
# **update_permission**
> PermissionResource update_permission(permission => $permission, permission_resource => $permission_resource)
...
eval { 
    my $result = $api_instance->update_permission(permission => $permission, permission_resource => $permission_resource);
    print Dumper($result);
};
if ($@) {
