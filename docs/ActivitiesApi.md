...
eval { 
    my $result = $api_instance->get_activities(filter_template => $filter_template, size => $size, page => $page, order => $order);
    print Dumper($result);
};
if ($@) {
...
# **update_activity**
> update_activity(id => $id, activity_resource => $activity_resource)
...
eval { 
    $api_instance->update_activity(id => $id, activity_resource => $activity_resource);
};
if ($@) {
