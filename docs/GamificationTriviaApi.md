...
eval { 
    my $result = $api_instance->get_question_tags1(filter_search => $filter_search, filter_category => $filter_category, filter_import_id => $filter_import_id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling GamificationTriviaApi->get_question_tags1: $@\n";
}
```
...
eval { 
    my $result = $api_instance->get_questions(size => $size, page => $page, order => $order, filter_search => $filter_search, filter_idset => $filter_idset, filter_category => $filter_category, filter_tagset => $filter_tagset, filter_type => $filter_type, filter_published => $filter_published, filter_import_id => $filter_import_id);
    print Dumper($result);
};
if ($@) {
...
> update_question_template(id => $id, question_template_resource => $question_template_resource)
...
eval { 
    $api_instance->update_question_template(id => $id, question_template_resource => $question_template_resource);
};
if ($@) {
