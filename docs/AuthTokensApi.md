...
# **delete_token**
> delete_token(username => $username, client_id => $client_id)
...
eval { 
    $api_instance->delete_token(username => $username, client_id => $client_id);
};
if ($@) {
    warn "Exception when calling AuthTokensApi->delete_token: $@\n";
}
```
...
# **delete_tokens**
> delete_tokens(username => $username)
...
eval { 
    $api_instance->delete_tokens(username => $username);
};
if ($@) {
