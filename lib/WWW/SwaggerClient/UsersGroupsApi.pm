...
# @param string $order A comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC] (optional, default to 1)
{
    ...
    __PACKAGE__->method_documentation->{ 'search_groups_using_get' } = { 
        ...
        params => $params,
        returns => 'PageGroupMemberResource',
        };
}
...
sub search_groups_using_get {
    ...
    my $_response_object = $self->{api_client}->deserialize('PageGroupMemberResource', $response);
    return $_response_object;
}
...
sub search_groups_using_get1 {
    ...
    my $_response_object = $self->{api_client}->deserialize('PageGroupResource', $response);
    return $_response_object;
}
