...
```perl
use KnetikCloud::Object::LogsApi;
```
...
eval { 
    my $result = $api_instance->get_bre_event_logs(filter_start_date => $filter_start_date, filter_event_name => $filter_event_name, filter_event_id => $filter_event_id, size => $size, page => $page, order => $order);
    print Dumper($result);
};
if ($@) {
...
eval { 
    my $result = $api_instance->get_user_logs(filter_user => $filter_user, filter_action_name => $filter_action_name, size => $size, page => $page, order => $order);
    print Dumper($result);
};
if ($@) {
