...
eval { 
    my $result = $api_instance->get_bre_triggers(filter_system => $filter_system, filter_category => $filter_category, filter_name => $filter_name, size => $size, page => $page);
    print Dumper($result);
};
if ($@) {
...
# **update_bre_trigger**
> update_bre_trigger(event_name => $event_name, bre_trigger_resource => $bre_trigger_resource)
...
eval { 
    $api_instance->update_bre_trigger(event_name => $event_name, bre_trigger_resource => $bre_trigger_resource);
};
if ($@) {
